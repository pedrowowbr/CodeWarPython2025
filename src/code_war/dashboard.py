...
if generos_selecionados:
    df = df[df["generos"].apply(lambda x: any(g in x.split(
        ",") for g in generos_selecionados) if pd.notnull(x) else False)]

# ---------------------- GALERIA DE CAPAS ----------------------
st.subheader(f"🎞️ Galeria de Filmes ({len(df)} encontrados)")
...
# ---------------------- GRÁFICOS ----------------------
st.subheader("📊 Relação entre Avaliação e Popularidade")

# Adiciona controles interativos para o gráfico
col1, col2 = st.columns(2)
with col1:
    tamanho_based = st.selectbox(
        "Tamanho dos pontos representa:",
        ["Quantidade de votos", "Ano de lançamento"],
        key="size_option"
    )

with col2:
    cor_based = st.selectbox(
        "Cor dos pontos representa:",
        ["Idioma original", "Gênero principal", "Ano de lançamento"],
        key="color_option"
    )

# Prepara os dados conforme seleção
...
df_filmes_ano = df.groupby(
    "ano_lancamento").size().reset_index(name="qtd_filmes")
fig2 = px.bar(
    df_filmes_ano,
    x="ano_lancamento",
    y="qtd_filmes",
    labels={"ano_lancamento": "Ano", "qtd_filmes": "Quantidade"},
...
# Média móvel de avaliações por ano
df_ano_media = df.groupby('ano_lancamento')[
    'media_votos'].mean().rolling(3).mean().reset_index()

fig_tendencia = px.line(
    df_ano_media,
    x='ano_lancamento',
    y='media_votos',
    title='Média Móvel (3 anos) das Avaliações',
    labels={'media_votos': 'Nota Média', 'ano_lancamento': 'Ano'}
)
